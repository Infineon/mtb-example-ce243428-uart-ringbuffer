# PDL: UART ring buffer

This code example receives UART data using a ring buffer and echoes it back in 8-byte chunks.

[View this README on GitHub.](https://github.com/Infineon/mtb-example-ce243428-uart-ringbuffer)

[Provide feedback on this code example.](https://yourvoice.infineon.com/jfe/form/SV_1NTns53sK2yiljn?Q_EED=)
...
The following example clones the "[mtb-example-ce243428-uart-ringbuffer](https://github.com/Infineon/mtb-example-ce243428-uart-ringbuffer)" application with the desired name "UartRingbuffer" configured for the *KIT_T2G_C-2D-6M_LITE* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_T2G_C-2D-6M_LITE --app-id mtb-example-ce243428-uart-ringbuffer --user-app-name UartTransmitReceive --target-dir "C:/mtb_projects"
   ```
...
4. After programming, the application starts automatically. Confirm that "UART Ring Buffer Demonstration" is displayed on the UART terminal. As shown in **Figure 1**, enter strings such as "12345678", and "abcdefgh". Confirm that the entered data is echoed back in 8-byte chunks
...
This code provides a system that stores received UART data in a ring buffer and echoes it back in 8-byte chunks, triggered by interrupt events.

Reception method:
1. The receive chunk size is fixed at 8 bytes (`UART_RECEIVE_CHUNK`)
2. An initial asynchronous receive request is issued
3. Upon receiving 8 bytes, a reception-completion event triggers a callback function, which transmits (echoes) the received data and sets a flag

Interrupt processing:
1. The ISR is kept minimal and simply invokes the PDL common handler
2. The actual event processing is performed within the callback function:
   - Reception complete: Echoes the received data and sets a flag
   - Reception error: Clears the RX FIFO
   - Ring buffer full: Requests the application to drain the received data

Main loop:
1. Operates by monitoring the flag and re-arms the next reception when the current reception is completion
2. Waits for transmission to complete before re-arming the receiver to avoid conflicts during transmission
...
Resources  | Links
-----------|----------------------------------
Application notes  | [AN235305](https://www.infineon.com/assets/row/public/documents/10/42/infineon-an235305-getting-started-with-traveo-t2g-family-mcus-in-modustoolbox-applicationnotes-en.pdf) – Getting started with TRAVEO&trade; T2G family MCUs in ModusToolbox&trade;　<br> [AN225401](https://www.infineon.com/row/public/documents/10/42/infineon-an225401---how-to-use-serial-communication-block-scb-in-traveo-t2g-family-applicationnotes-en.pdf) – How to use serial communications block (SCB) in TRAVEO&trade; T2G family
...
Document title: *CE243428* – *PDL: UART ring buffer* 
